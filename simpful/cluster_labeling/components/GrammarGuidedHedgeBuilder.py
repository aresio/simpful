...
class GrammarGuidedHedgeBuilder:
    def __init__(self, sets_to_modify: List[FuzzySet],
        ...
        self._tqdm: bool = use_tqdm

    def __generate_hedge_population(self, num_individuals: int = -1) -> List[HedgeChain]:
        """
        ...
        return self.cfg.generate_individuals(n=num_individuals)
    ...
    @staticmethod
    def __get_top_k_candidates(population: List[Individual], k: int = 5) -> List[Individual]:
    ...
    def __find_best_compromise(self, sorted_best_candidates: List[Individual]) -> Individual:
        ...
        """
        # TODO [IMPROVEMENT]: Add different criteria
    ...
    def run_naive_best(self, num_individuals: int = -1) -> FuzzySet:
        ...
        # Calculate fitness
        chain_population_with_fitness: List[Individual] = self.__hedge_population_fitness(population)
        # Get the k top fit candidates (default 5)
        elite_candidates: List[Individual] = self.__get_top_k_candidates(chain_population_with_fitness)
    ...
    def run_conservative(self, num_individuals: int = -1,
        ...
        """
        Run conservative approach.
        Best hedges in terms of fitness with a compromise (e.g., shorter are preferred)
        ...
        while True:
            ...
            # Otherwise, find the best candidate in the list (one will be met eventually)
            elif len(filtered_candidates) >= minimum_k_candidates or accept_similarity_threshold <= 0:
    ...
    def run_complete(self, num_individuals: int = -1,
        ...
        Like conservative, but keeps template as is if it already satisfied similarity threshold
        ...
        for basic_template in self.sets_to_modify:
            ...
            # If base template is already good enough or better than others
            if sim >= accept_similarity_threshold and best_similarity < sim:
                best_similarity = sim
        ...
        if accepted_template is not None:
