...
class LinguisticVariable(object):
    ...
    def draw(self, ax, TGT=None, element=None, highlight=None, xscale="linear"):
        ...
        if TGT is not None:
            print(
                "Warning: argument 'TGT' is deprecated and will be discontinued in future relseases. Please use 'element'"
            )
            element = TGT
    ...
    def plot(
        self, outputfile="", TGT=None, element=None, highlight=None, xscale="linear"
    ):
        """
...
class FuzzySystem(object):
    ...
    def __init__(
        self, operators=None, show_banner=False, sanitize_input=False, verbose=False
    ):
    ...
    def replace_rule(self, i, new_rule, verbose=False):
        ...
        parsed_antecedent = recursive_parse(
            preparse(new_rule), verbose=verbose, operators=self._operators
        )
    ...
    def _replace_values(self, function, verbose=False):
        ...
        while "{" in res_string:
            ...
            try:
                ...
                if verbose:
                    print("   Pre- and sub-strings:", prestring, substring)
            except:
            ...
            case = float(
                substring[substring.find("IS") + 2 : substring.find("THEN")].strip()
            )
            value = substring[substring.find("THEN") + 4 :].strip()
            if verbose:
                print(
                    " * Analysing rule: IF %s IS %s THEN %s" % (variable, case, value)
                )
            ...
            if candidate == case:
                ...
                if verbose:
                    print(" - case detected for '%s IS %s" % (variable, case))
            else:
                res_string = res_string[res_string.find("}") + 1 :]
                newstring += prestring + "0"
                if verbose:
                    print(" - case NOT detected for '%s IS %s'" % (variable, case))
                ...
                if outname == output:
                    ...
                    if crisp:
                        ...
                        if self._check_templates() == TEMPLATES_ENGAGED:
                            string_to_evaluate = self._replace_values(
                                string_to_evaluate, verbose=verbose
                            )
                            if verbose:
                                print(
                                    " * Replacing templates in function for '%s'"
                                    % res[0]
                                )
                                print("   name of function: '%s'" % res[1])
                                print(
                                    " * Final version of the '%s' rule: %s"
                                    % (res[1], string_to_evaluate)
                                )
    ...
    def Sugeno_inference(
        self, terms=None, ignore_errors=False, ignore_warnings=True, verbose=False
    ):
        """
    ...
    def plot_variable(
        ...
        if ax != None:
            ax = self._lvs[var_name].draw(
                ax=ax, element=element, highlight=highlight, xscale=xscale
            )
            return ax
        self._lvs[var_name].plot(
            outputfile=outputfile, element=element, highlight=highlight, xscale=xscale
        )
    ...
    def produce_figure(self, outputfile="", max_figures_per_row=4, element_dict=None):
        ...
        if num_ling_variables == 0:
            raise Exception(
                "ERROR: no linguistic variables added. Please add at least one variable to plot"
            )
    ...
    def plot_surface(
        self, variables, output, detail=40, color_map="plasma", outputfile=""
    ):
        """
