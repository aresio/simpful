...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Genetic Programming Simulation for Evolving Fuzzy Systems"
    )
    parser.add_argument(
        "--x_train", type=str, required=True, help="Path to the training data (X)."
    )
    parser.add_argument(
        "--y_train", type=str, required=True, help="Path to the training data (Y)."
    )
    parser.add_argument(
        "--terms_dict_path",
        type=str,
        required=True,
        help="Path to the terms dictionary file.",
    )
    parser.add_argument(
        "--exclude_columns",
        type=str,
        default="month,day,hour",
        help="Columns to exclude, separated by commas.",
    )
    parser.add_argument(
        "--population_size", type=int, default=100, help="Size of the population."
    )
    parser.add_argument(
        "--max_generations", type=int, default=50, help="Maximum number of generations."
    )
    parser.add_argument(
        "--mutation_rate", type=float, default=0.01, help="Mutation rate."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="tournament",
        help="Selection method ('tournament' or 'roulette').",
    )
    parser.add_argument(
        "--tournament_size", type=int, default=3, help="Tournament size for selection."
    )
    parser.add_argument("--elitism_rate", type=float, default=0.1, help="Elitism rate.")
    parser.add_argument(
        "--max_rules", type=int, default=7, help="Maximum number of rules per system."
    )
    parser.add_argument(
        "--min_rules", type=int, default=3, help="Minimum number of rules per system."
    )
    parser.add_argument(
