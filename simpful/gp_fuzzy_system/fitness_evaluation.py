...
def weighted_rmse(actual, predicted, verbose=False):
    ...
    if verbose:
        ...
        print(
            f"Inside weighted_rmse - Type of actual: {type(actual)}, shape: {actual.shape}"
        )
        print(
            f"Inside weighted_rmse - Type of predicted: {type(predicted)}, shape: {predicted.shape}"
        )
...
def evaluate_fitness(system, predictions, actual, weights=None):
    ...
    normalized_stability = (
        1 - stability_score
    )  # Assuming we want to minimize instability (maximize stability)
    normalized_utility = 1 / (
        1 + utility_score
    )  # Assuming we want to minimize utility, and utility_score > 0
    normalized_zero_penalty = (
        zero_penalty_score  # Higher penalty for more zero predictions
    )
