...
class TestRuleProcessor(unittest.TestCase):
    ...
    def test_strip_parentheses(self):
        ...
        expected = (
            "IF PaO2 IS high AND Trombocytes IS low THEN Sepsis IS high_probability"
        )
...
class TestExtractFeatureTerm(unittest.TestCase):
    ...
    def test_extract_feature_term(self):
        parsed_outputs = [
            ...
            [
                ("trade_balance_value", "Low"),
                ("foreign_direct_investment_value", "Low"),
            ],
        ...
        for rule, expected in zip(expected_output, parsed_outputs):
            with self.subTest(rule=rule):
                self.assertEqual(
                    extract_feature_term(rule, self.available_features), expected
                )
